...
def create_dataset_entries(
    ...
    # Let's convert the generated_images dict from bytes into HuggingFace Image format
    generated_hf_images = {
        category: HFImage(image_bytes)
        for category, image_bytes in generated_images.items()
    }
    ...
    for sentence, ordering_rules, sentence_type in zip(
        ...
        ordered_images: list[HFImage] = [
            generated_hf_images[category] for category in ordering_rules
        ]
